            ...
            <li v-for="(items,index) in ScoreDB.scoreDatas" :key="index">
                ...
                <nav>     
                    <a href="javascript:;"  v-for="(item ,index) in items.scoreArr"
                     @click="scoreFuc(index+1, items.scoreFlag,items.scoreArr)" :class="[item==index ? 'xw-score-active' : 'xw-score-notActiv']" :key="index"></a>
                </nav>
...
.xw-score-list li span{
 ...
 nav a{
      float: left;
      background: url(../assets/images/star.png) no-repeat -38px 0;
      width: 38px;
      height: 36px;
      margin-right: 10px;
      transition: .3s;
